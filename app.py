...
if page == "Chatbot":
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        with st.chat_message("assistant"):
            ...
            json_answer = pipeline(query, api_list, args_, tools_, allowed_args_dict,
                                   retrieval_vector_db)  # allowed args dict ka placeholder modify karna bacha
    ...
    with st.form("new_tool_form"):
        ...
        if submitted:
            st.session_state.api_list_updated = add_tool(st.session_state.api_list_updated, new_tool_name,
                                                         new_tool_desc)
    ...
    if st.session_state.api_list_updated:
        selected_tool_name = st.selectbox("Select a tool",
                                          options=[tool["name"] for tool in st.session_state.api_list_updated])
        selected_tool = next((tool for tool in st.session_state.api_list_updated if tool['name'] == selected_tool_name),
                             None)
        ...
        if selected_tool:
            with st.form("update_delete_tool_form"):
                ...
                if update_button:
                    st.session_state.api_list_updated = update_tool(st.session_state.api_list_updated,
                                                                    selected_tool_name, new_name, new_description)
                    st.success("Tool Updated Successfully!")

                if delete_button:
                    st.session_state.api_list_updated = delete_tool(st.session_state.api_list_updated,
                                                                    selected_tool_name)
    ...
    if st.session_state.api_list_updated:  # Check if there are tools available
        selected_tool_name = st.selectbox("Select a tool to add an argument",
                                          options=[tool["name"] for tool in st.session_state.api_list_updated])
        with st.form("new_argument_form"):
            ...
            if submitted_arg:
                st.session_state.api_list_updated = add_argument(st.session_state.api_list_updated, selected_tool_name,
                                                                 new_arg_name, new_arg_desc, new_arg_type)
    ...
    if selected_tool:
        st.write(f"Selected Tool: {selected_tool_name}")
        selected_arg_name = st.selectbox("Select an argument",
                                         options=[arg["argument_name"] for arg in selected_tool["arguments"]])
        selected_arg = next((arg for arg in selected_tool['arguments'] if arg['argument_name'] == selected_arg_name),
                            None)
        ...
        if selected_arg:
            with st.form("update_delete_arg_form"):
                ...
                if f"{selected_tool_name}/{selected_arg_name}" in arg_allowed_values_dict:
                    new_arg_allowed_values = st.text_area("New Argument Allowed Values", value=arg_allowed_values_dict[
                        f"{selected_tool_name}/{selected_arg_name}"])
                else:
                ...
                if update_arg_button:
                    st.session_state.api_list_updated = update_argument(st.session_state.api_list_updated,
                                                                        selected_tool_name, selected_arg_name,
                                                                        new_arg_name, new_arg_desc, new_arg_type)
                    arg_allowed_values_dict[f"{selected_tool_name}/{new_arg_name}"] = new_arg_allowed_values
                    st.success("Argument Updated Successfully!")

                if delete_arg_button:
                    st.session_state.api_list_updated = delete_argument(st.session_state.api_list_updated,
                                                                        selected_tool_name, selected_arg_name)
    ...
    if selected_tool:
        ...
        if st.button("Delete Selected Arguments"):
            st.session_state.api_list_updated = delete_multiple_arguments(st.session_state.api_list_updated,
                                                                          selected_tool_name, selected_args_to_delete)
